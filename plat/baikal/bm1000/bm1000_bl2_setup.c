...
void bl2_platform_setup(void)
{
	...
	if (ret >= 0) {
		...
		for (dimm_idx = 0; dimm_idx < ARRAY_SIZE(dimm_spd_addrs); ++dimm_idx) {
			struct ddr4_spd_eeprom *spd = (struct ddr4_spd_eeprom *)(spd_ptr + dimm_idx * 512);
		...
		flush_dcache_range((unsigned long)fdt, PLAT_BAIKAL_DTB_MAX_SIZE);
	} else {
...
int bl2_plat_handle_post_image_load(unsigned int image_id)
{
	...
	case BL32_IMAGE_ID:
		 ...
		 */
		bl_mem_params->ep_info.args.arg3 = PLAT_BAIKAL_SEC_DTB_BASE;
#endif
		/*
	...
	case BL33_IMAGE_ID:
		...
		flush_dcache_range((uintptr_t)PLAT_BAIKAL_NS_DTB_BASE, PLAT_BAIKAL_DTB_MAX_SIZE);
