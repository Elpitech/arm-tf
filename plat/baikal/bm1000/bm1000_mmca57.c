...
#define MMCA57_SCLKEN_REG		0x0808
...
unsigned mmca57_get_sclk_div(const uintptr_t base)
{
	...
	reg = mmio_read_32(base + MMCA57_SCLKEN_REG);
...
void mmca57_enable_core(const u_register_t mpidr)
{
	...
	reg = mmio_read_32(base + MMCA57_CFG_RST_REG);
	reg |= (MMCA57_CFG_RST_CORERESET0 |
		MMCA57_CFG_RST_CPUPORESET0) << core;
	mmio_write_32(base + MMCA57_CFG_RST_REG, reg);
	udelay(1);

	/* Deassert cluster resets */
	reg &= ~(MMCA57_CFG_RST_L2RESET   |
...
void mmca57_reconf_sclken(const uintptr_t base, const unsigned div)
{
	...
	mmio_write_32(base + MMCA57_SCLKEN_REG, reg);
	reg &= ~MMCA57_SCLKEN_VAL_DIV_MASK;
	...
	if (cmu_pll_is_enabled(base)) {
		...
		while (!(mmio_read_32(base + MMCA57_SCLKEN_REG) & MMCA57_SCLKEN_LOCK_DIV)) {
	...
	mmio_setbits_32(base + MMCA57_SCLKEN_REG, MMCA57_SCLKEN_EN);
}
