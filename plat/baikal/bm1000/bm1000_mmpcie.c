...
void mmpcie_init(void)
{
	...
	cmu_clkch_enable_by_base(MMPCIE_CMU0_CLKCHCTL_TBU0_SLV,	   4); /* 375 MHz */
	cmu_clkch_enable_by_base(MMPCIE_CMU0_CLKCHCTL_TBU1_SLV,	   4); /* 375 MHz */
	cmu_clkch_enable_by_base(MMPCIE_CMU0_CLKCHCTL_TBU2_SLV,	   3); /* 500 MHz */

	/* Deassert reset signals */
	reg = mmio_read_32(MMPCIE_GPR_MMRST);
	reg &= ~(MMPCIE_GPR_MMRST_NIC_CFG_PCIE0 |
		 MMPCIE_GPR_MMRST_NIC_CFG_PCIE1 |
		 MMPCIE_GPR_MMRST_NIC_CFG_PCIE2 |
		 MMPCIE_GPR_MMRST_NIC_CFG_TCU);
	mmio_write_32(MMPCIE_GPR_MMRST, reg);
	reg &= ~MMPCIE_GPR_MMRST_NIC_CFG_SLV;
	mmio_write_32(MMPCIE_GPR_MMRST, reg);
	reg &= ~(MMPCIE_GPR_MMRST_NIC_SLV_PCIE0 |
		 MMPCIE_GPR_MMRST_NIC_SLV_PCIE1 |
		 MMPCIE_GPR_MMRST_NIC_SLV_PCIE2);
	mmio_write_32(MMPCIE_GPR_MMRST, reg);
	reg &= ~MMPCIE_GPR_MMRST_NIC_SLV_SLV;
	mmio_write_32(MMPCIE_GPR_MMRST, reg);
	...
	mmio_write_32(MMPCIE_NIC_CFG_SECURITY_SMMU,  NIC_SECURITY_REGION0_NONSECURE);
	mmio_write_32(MMPCIE_NIC_CFG_SECURITY_PCIE0, NIC_SECURITY_REGION0_NONSECURE);
	mmio_write_32(MMPCIE_NIC_CFG_SECURITY_PCIE1, NIC_SECURITY_REGION0_NONSECURE);
	mmio_write_32(MMPCIE_NIC_CFG_SECURITY_PCIE2, NIC_SECURITY_REGION0_NONSECURE);

	mmio_write_32(MMPCIE_NIC_SECURITY_PCIE0, NIC_SECURITY_REGION0_NONSECURE);
	mmio_write_32(MMPCIE_NIC_SECURITY_PCIE1, NIC_SECURITY_REGION0_NONSECURE);
	mmio_write_32(MMPCIE_NIC_SECURITY_PCIE2, NIC_SECURITY_REGION0_NONSECURE);

	/* Enable non-secure access to some GPRs */
	mmio_write_32(MMPCIE_GPRS_BASE + 0x00,
	...
	mmio_write_32(MMPCIE_GPRS_BASE + 0x04,
