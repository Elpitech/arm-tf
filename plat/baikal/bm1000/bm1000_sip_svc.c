...
#define BAIKAL_SMC_CMU_PLL_RESET	13
#define BAIKAL_SMC_CMU_CLKCH_RESET	14
	...
	while (1) {
		...
		if (!fdt_node_check_compatible(fdt, offset, "baikal,bm1000-cmu")) {
			...
			if (cmu->base != MMAVLSP_CMU1_BASE &&
			    cmu->base != MMDDR0_CMU0_BASE  &&
			    cmu->base != MMDDR1_CMU0_BASE  &&
			    cmu->base != MMXGBE_CMU1_BASE) {
				cmu_pll_set_rate(cmu->base, cmu->frefclk, fpllreq);
			}

			INFO("cmu@%lx clk=%lu freq=%lu reconf=%d chans=%d\n",
			     cmu->base,
			     cmu->frefclk,
			     fpllreq,
			     cmu->deny_pll_reconf,
...
static uint64_t baikal_smc_pm_handler(void)
{
	mmio_write_32(MMMALI_NIC_CFG_SECURITY_MALI, NIC_SECURITY_REGION0_NONSECURE);
	mmio_write_32(MMMALI_NIC_CFG_SECURITY_SMMU, NIC_SECURITY_REGION0_NONSECURE);
				 ...
				 u_register_t flags)
{
	...
	case BAIKAL_SMC_CMU_CMD:
		...
		case BAIKAL_SMC_CMU_CLKCH_IS_ENABLED:
			...
			break;
		case BAIKAL_SMC_CMU_PLL_RESET:
			ret = cmu_pll_reset(x1);
			break;
		case BAIKAL_SMC_CMU_CLKCH_RESET:
			ret = cmu_clkch_reset(x4, x1);
			break;
		default:
