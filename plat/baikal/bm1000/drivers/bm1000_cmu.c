...
#define NF_MIN					0x0000200000000UL
#define NF_MAX					0x80001ffffffffUL
...
static void cmu_pllvals_get(uintptr_t base, struct pllvals *pllvals);
static int cmu_pllvals_calc(uint64_t frefclk, uint64_t fpllreq, struct pllvals *pllvals);
static int cmu_pllvals_set(const struct cmu_desc *cmu, uint64_t frefclk, uint64_t fpllreq);
static int cmu_clkch_div_calc(const struct cmu_desc *cmu, unsigned int clkch, uint64_t fclkchreq);
static int cmu_clkch_div_get(const struct cmu_desc *cmu, unsigned int clkch);
static uintptr_t cmu_clkch_get_reg(uintptr_t base, unsigned int clkch);
static int cmu_clkch_div_set(const struct cmu_desc *cmu, unsigned int clkch, uint32_t div);
static struct cmu_desc *cmu_desc_get_by_base(uintptr_t base);
static int cmu_pll_lock_debounce(uintptr_t base);
static int cmu_set_core_rate(const struct cmu_desc *cmu, uint64_t frefclk, uint64_t fpllreq);
static int cmu_set_periph_rate(const struct cmu_desc *cmu, uint64_t frefclk, uint64_t fpllreq);
static void cmu_pll_set_divs(uintptr_t base, unsigned int nr, unsigned int od);
static void cmu_pll_set_nf(uintptr_t base, uint64_t nf);
...
int64_t cmu_pll_get_rate(const uintptr_t base, uint64_t frefclk)
{
	...
	cmu_pllvals_get(base, &pllvals);
	fpll = (frefclk * pllvals.intnf) / (pllvals.nr * pllvals.od);
...
int64_t cmu_pll_round_rate(const uintptr_t base, uint64_t frefclk, const uint64_t fpllreq)
{
	...
	ret = cmu_pllvals_calc(frefclk, fpllreq, &pllvals);
	if (ret) {
		return ret;
	}
...
{
	VERBOSE("%s: base=0x%08lx reset\n", __func__, base);
	mmio_setbits_32(base + CMU_PLL_CTL6, CMU_PLL_CTL6_SWRST);
	return 0;
}
...
int cmu_clkch_reset(uintptr_t base, unsigned int clkch)
{
	...
	VERBOSE("%s: base=0x%08lx ch=%u reset\n", __func__, base, clkch);
	mmio_setbits_32(clkch_reg, CMU_CLKCH_CTL_SWRST);
	return 0;
}

static void cmu_pllvals_get(const uintptr_t base, struct pllvals *const pllvals)
{
	uint64_t clkflo;
	uint64_t clkfhi;

	pllvals->od = ((mmio_read_32(base + CMU_PLL_CTL0) &
		      CMU_PLL_CTL0_CLKOD_MASK) >> CMU_PLL_CTL0_CLKOD_SHIFT) + 1;

	pllvals->nr = ((mmio_read_32(base + CMU_PLL_CTL0) &
	...
	pllvals->intnf = ((clkfhi << CLKFHI_SHIFT) + (clkflo << CLKFLO_SHIFT)) >> NF_INT_SHIFT;

	VERBOSE("%s: base=0x%08lx nr=%u od=%u nf=%u\n",
		__func__, base, pllvals->nr, pllvals->od, pllvals->intnf);
}

static int cmu_pllvals_calc(const uint64_t frefclk,
			    const uint64_t fpllreq,
			    struct pllvals *const pllvals)
{
	if (!frefclk || !fpllreq || !pllvals) {
		return -ENXIO;
	}
...
static int cmu_pllvals_set(const struct cmu_desc *const cmu,
			   uint64_t frefclk,
			   const uint64_t fpllreq)
{
	int ret;
	struct pllvals current;
	struct pllvals calc;
	...
	cmu_pllvals_get(cmu->base, &current);
	ret = cmu_pllvals_calc(frefclk, fpllreq, &calc);
	if (ret) {
		return ret;
	}
	...
	if (current.nr != calc.nr || current.od != calc.od) {
		cmu_pll_set_divs(cmu->base, calc.nr, calc.od);
	}
			     ...
			     const uint64_t fpllreq)
{
	...
	err = cmu_pllvals_set(cmu, frefclk, fpllreq);
	if (err) {
		return err;
	}
...
{
	static struct cmu_desc cmus[16];
...
static void cmu_pll_set_divs(const uintptr_t base,
			     ...
			     const unsigned int od)
{
	...
	/* Simultaneously modify NR and OD dividers */
	mmio_clrsetbits_32(base +
		CMU_PLL_CTL0,
