...
static void cfg_check(struct ddr_local_conf *conf, bool two_dimms)
{
	int dimms = two_dimms?2:1;
...
{
	conf->crc = crc16(conf, offsetof(struct ddr_local_conf, crc), 0);
	ddr_write_conf(idx?2:0, conf, sizeof(*conf));
}
...
int ddr_conf(struct ddr_local_conf *conf0, struct ddr_local_conf *conf1,
		unsigned ddr_map)
{
	int key = 0;
	uint64_t timeout;
	...
	if (!conf0 && !conf1)
		return 0;
	if (conf0)
		cfg_check(conf0, ddr_map & DIMM1_PRESENT);
	if (conf1)
		cfg_check(conf1, ddr_map & DIMM3_PRESENT);
