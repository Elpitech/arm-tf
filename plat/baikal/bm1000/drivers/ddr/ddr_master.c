...
static inline uint16_t set_mr0(uint32_t CL, uint32_t WR, uint32_t RTP)
{
	...
	/*
	 * The WR value in the mode register must be programmed
	 * to be equal or larger than WRmin
	 */
	WR = (WR + 1) & ~0x1U;
 ...
 */
static inline uint16_t set_mr2(uint32_t CWL, uint32_t RTT_WR)
{
 ...
 */
static inline uint16_t set_mr5(uint32_t PL, uint32_t RTT_PARK)
{
...
void ddrc_set_default_vals(struct ctl_content *ddrc)
{
	...
	ddrc->INIT3_	  = 0x0a34050b;
	ddrc->INIT4_	  = 0x10280200;
	ddrc->INIT6_	  = 0x00000402;
	ddrc->INIT7_	  = 0x00000800;
			...
			const struct ddr_configuration *data)
{
	if (data->single_ddr) {
		...
		ddrc->SARBASE0_ *= 2; /* region address 2..4 GB (UMCTL2_SARMINSIZE=256MB) */
		ddrc->SARSIZE0_ = (ddrc->SARSIZE0_ + 1) * 2 - 1; /* region size 2 GB */
		ddrc->SARBASE1_ *= 2; /* region address 34..64 GB */
		ddrc->SARSIZE1_ = (ddrc->SARSIZE1_ + 1) * 2 - 1; /* region size 30 GB */
		ddrc->SARBASE2_ *= 2; /* region address 544..1024 GB */
		ddrc->SARSIZE2_ = (ddrc->SARSIZE2_ + 1) * 2 - 1; /* region size 240 GB */
	}
	 ...
	 * COL11,10    ->none
	 * BA1,0       ->HIF A12,A11 */
	uint32_t hif_addr = 13; /* next empty HIF address */
	if (data->dbus_half) {
	...
	/* DIMM_0 have 00b and 01b rank addresses on the DBM board;
	 * DIMM_1 have 10b and 11b rank addresses on the DBM board */
	if (data->ranks == 2) {
		ddrc->ADDRMAP0_ &= ~GENMASK(4, 0);
	...
	ddrc->INIT4_ &= ~GENMASK(15, 0);
	ddrc->INIT4_ |=  GENMASK(15, 0) & (((uint8_t)data->WCL - 4) << 9);
	/* Per DRAM Addressability off; Geardown Mode 1/2; MPR operation normal; MPR Page 0; Set Write Command Latency */
	/* Write Preamble 1clk; Read Preable 1 clk; Read Preable Training off; Self Refresh Abort off; */
	ddrc->INIT6_ &= ~GENMASK(31, 16);
	/* Command Address Latency off; Vref Monitor off; Temp Controlled Refresh off; Max Power Down mode off */
	/* Read DBI off; Write DBI off; DM on; CA Parity Persistent Error off; RTT_PARK=off; */
	ddrc->INIT6_ &= ~GENMASK(15, 0);
	ddrc->INIT6_ |= set_mr5(data->PL, data->RTT_PARK);
	/* ODT input buffer PD on; CA Parity Error clear; CRC Error clear; set CA Parity Latency Mode */
	/* VrefDQ Training off; set tCCD_L */
	ddrc->INIT7_ &= ~GENMASK(15, 0);
	ddrc->INIT7_ |=  GENMASK(15, 0) & ((data->tCCD_L - 4) << 10);
	...
	/* DDR4 multiPHY Utility Block (PUB) Databook, 4_ 9.4 Read Transactions, Table 4-23 Read Command Spacing:
	 * add extra Read System Latency in Read2Write delay */
	uint32_t RSL = (data->clock_mhz * 2 <= 1600) ? 4 : 6; /* experimental RSL = 4 or 6 clk */
		       ...
		       const struct ddr_configuration *data)
{
	...
	phy->MR6_ = (data->tCCD_L - 4) << 10; /* VrefDQ Training off; set tCCD_L */
...
#ifdef DDR4_USE_ASYM_DRV
	phy->ZQCR_ |= 0x1 << 24; /* Program ZQCR to set pull-up-only termination and asymmetric drive */
	phy->ZQCR_ |= 0x1 << 25; /* see "dwc_ddr4_multiphy_pub_databook.pdf": */
	phy->ZQCR_ |= ~(1 << 26); /* 5.4 DDR Protocol-Specific Usage Models 5.4.1 DDR4 Usage */

	if (data->PHY_ODI_PU) {
		phy->ZQ1PR_ &= ~GENMASK(11, 8);
		phy->ZQ1PR_ |=  GENMASK(11, 8) & (data->PHY_ODI_PU << 8);
	}
	if (data->PHY_ODI_PD) {
		phy->ZQ1PR_ &= ~GENMASK(15, 12);
		phy->ZQ1PR_ |=  GENMASK(15, 12) & (data->PHY_ODI_PD << 12);
	}
	if (data->PHY_ODT) {
		phy->ZQ1PR_ &= ~GENMASK(19, 16);
		phy->ZQ1PR_ |=  GENMASK(19, 16) & (data->PHY_ODT << 16);
	}
#else
	if (data->PHY_ODI_PU) {
		phy->ZQ1PR_ &= ~0xf;
		      ...
		      struct ddr_configuration *data)
{
	...
	if (data->row_address > 18 || data->row_address < 14) {
		ERROR("unsupported row adress width (%d)\n", data->row_address);
		return -1;
	}
	...
	case DDR4_SPD_MODULETYPE_72B_SO_RDIMM:
		...
		data->registered_ca_stren =
			spd->mod_section.registered.ca_stren;
		data->registered_clk_stren =
			spd->mod_section.registered.clk_stren;
		break;
	 ...
	 *
	 * FTB - fine timebase
	 * The FTB in the SPD spec is 1ps
	 */
	if (spd->timebases) {
	...
	case 3200:
		...
		break;
	default:
		lat_mask = 0ULL; /* unsupported DDR clock */
		break;
	}
	...
	/*
	 * Compute C/A Parity Latency (PL)
	 * JEDEC 79-4, page 21, table 12
	 */
	if (data->clock_mhz * 2 < 1600) {
		data->PL = 0;
	...
	tmp = SPD_TO_PS(((spd->tras_trc_ext & 0xf) << 8) | spd->tras_min_lsb,
			0);
	...
	tmp = SPD_TO_PS(((spd->tras_trc_ext & 0xf0) << 4) | spd->trc_min_lsb,
			spd->fine_trc_min);
	data->tRC = CLOCK_PS(tmp);

	/* Get Write to Read Time (tWTR_L), same bank group */
	tmp = SPD_TO_PS(((spd->twtr_min_ext & 0xf0) << 4) | spd->twtrl_min_lsb,
			0);
	data->tWTR_L = CLOCK_PS(MAX(tmp, 7500U));

	/* Get Write to Read Time (tWTR_S), different bank group */
	tmp = SPD_TO_PS(((spd->twtr_min_ext & 0xf) << 8) | spd->twtrs_min_lsb,
			0);
	...
	data->tFAW = CLOCK_PS(tmp);

	/* Get Average Periodic Refresh Interval (tREFI) */
	data->tREFI = CLOCK_NS(7800);

	/* Compute DLL Locking time (tDLLK) */
	/* DLL lock. JEDEC 79-4, page 22, table 13 */
	...
	/*
	 * Compute Mode Register Set command cycle time
	 * in PDA mode (tMRD_PDA)
	 */
	...
	data->tCCD_L = CLOCK_PS(tmp);
	if (data->clock_mhz * 2 <= 1600) {
		tmp = MAX(5U, CLOCK_PS(6250));
	} else if (data->clock_mhz * 2 <= 2132) {
		tmp = MAX(5U, CLOCK_PS(5355));
	} else {
		tmp = MAX(5U, CLOCK_NS(5));
	}
	data->tCCD_L = MAX(data->tCCD_L, tmp);
	...
	/*
	 * Compute Exit Self Refresh to commands not
	 * requiring a locked DLL (tXS)
	 */
	...
	/*
	 * Compute SRX to commands not requiring a locked DLL
	 * in Self Refresh ABORT (tXS_ABORT)
	 */
	data->tXS_ABORT = data->tRFC4 + CLOCK_NS(10);

	/*
	 * Compute Exit Self Refresh to tXSDLL commands
	 * requiring a locked DLL (tXSDLL)
	 */
