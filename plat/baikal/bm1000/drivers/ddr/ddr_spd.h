...
/* From JEDEC Standard No. 21-C, Release 30 */
struct ddr4_spd_eeprom {
	...
	uint8_t twr_min_ext;		/* 41 Upper Nibble for tWRmin, MTB */
	uint8_t twr_min_lsb;		/* 42 tWRmin, MTB */
	uint8_t twtr_min_ext;		/* 43 Upper Nibbles for tWTRmin, MTB */
	uint8_t twtrs_min_lsb;		/* 44 tWTR_Smin, MTB */
	uint8_t twtrl_min_lsb;		/* 45 tWTR_Lmin, MTB */
	uint8_t res_46[60 - 46];	/* 46 Reserved */
...
struct spd_container {
	struct ddr4_spd_eeprom content[2];
	struct {
...
void *ddr_read_spd(unsigned int dimm_idx);
