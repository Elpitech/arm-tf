...
static const unsigned char power_domain_tree_desc[] = {
	PLATFORM_CLUSTER_COUNT * PLATFORM_CHIP_COUNT,
...
CASSERT(ARRAY_SIZE(power_domain_tree_desc) ==
	PLATFORM_CLUSTER_COUNT * PLATFORM_CHIP_COUNT + 1,
...
{
	unsigned int chip_id, cluster_id, cpu_id, thread_id;

	chip_id    = MPIDR_AFFLVL3_VAL(mpidr);
	cluster_id = MPIDR_AFFLVL2_VAL(mpidr);
	cpu_id	   = MPIDR_AFFLVL1_VAL(mpidr);
	thread_id  = MPIDR_AFFLVL0_VAL(mpidr);

	if (chip_id >= PLATFORM_CHIP_COUNT) {
		return -1;
	}
