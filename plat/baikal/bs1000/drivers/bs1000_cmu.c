...
static int reg_write(uintptr_t addr, uint32_t  val);
static int reg_read (uintptr_t addr, uint32_t *val);
...
{
	static struct clk_desc clks[400 * PLATFORM_CHIP_COUNT]; /* TODO: allocate memory */
...
static int reg_write(uintptr_t addr, uint32_t val)
{
	...
	if (addr >= PLATFORM_ADDR_OUT_CHIP(chip, SC_LCRU_BASE) &&
	    addr <  PLATFORM_ADDR_OUT_CHIP(chip, SC_LCRU_BASE) + SC_LCRU_SIZE) {
		int ret = sc_lcru_write(addr, val);
...
{
	unsigned int chip = PLATFORM_ADDR_CHIP(base);
	int64_t freq;
	uint32_t raw;
	int ret;

	ret = reg_read(PLATFORM_ADDR_OUT_CHIP(chip, SC_GPR_SCP_BOOT_INFO_STS),
		       &raw);
#ifdef BAIKAL_QEMU_S
	ret = 0;
	raw = 0;
	...
	if (raw & SC_GPR_SCP_BOOT_INFO_STS_REFCLK) {
		/* REFCLK = 1: ref_clk0, ref_clk1 = 100 MHz */
		freq = 100 * MHZ;
	} else {
		/* REFCLK = 0: ref_clk0, ref_clk1 = 25 MHz */
		freq = 25 * MHZ;
	}
				...
				const uint64_t x4)
{
	...
	switch (PLATFORM_ADDR_IN_CHIP(addr)) {
	case GMAC0_BASE:
		mask = SC_GPR_GMAC_DIV_CTL_GMAC0_DIV2_EN;
		break;
	case GMAC1_BASE:
		mask = SC_GPR_GMAC_DIV_CTL_GMAC1_DIV2_EN;
		break;
	default:
		return -1;
	}
	...
	case BAIKAL_SMC_GMAC_DIV2_ENABLE:
		reg_read(PLATFORM_ADDR_OUT_CHIP(chip, SC_GPR_GMAC_DIV_CTL), &reg);
		reg |= mask;
		return reg_write(PLATFORM_ADDR_OUT_CHIP(chip, SC_GPR_GMAC_DIV_CTL), reg);
