...
int reg_write (uintptr_t addr, uint32_t  value);
int reg_read  (uintptr_t addr, uint32_t *value);
...
struct clk_ops {
	...
	int	(*set		) (uintptr_t base, int64_t freq);
	int	(*disable	) (uintptr_t base);
	int	(*enable	) (uintptr_t base);
	int	(*is_enabled	) (uintptr_t base);
...
struct clk_desc *cmu_desc_create(void *fdt, int offs, int index)
{
	...
	/* base */
	ac = fdt_getprop(fdt, offs, "reg", &plen);
	if (plen <= 0){
		return NULL;
	}
	base = fdt32_to_cpu(ac[0]);

	/* index */
	ac = fdt_getprop(fdt, offs, "clock-indices", &plen);
	int cnt0 = plen/sizeof(uint32_t);
	...
	if (cnt0 > 0){
		base += fdt32_to_cpu(ac[index]) * 0x10;
	} else if (cnt1 > 0){
		base += index * 0x10;
	} else {
		// base = base;
	...
	/* type */
	ac = fdt_getprop(fdt, offs, "type", &plen);
	if (plen > 0) {
		clk->type = fdt32_to_cpu(ac[0]);
	}
	switch (clk->type) {
		case CLK_TYPE_REFCLK:   clk->ops = &ops_clkref;  break;
		case CLK_TYPE_PLL:      clk->ops = &ops_pll;     break;
		case CLK_TYPE_CLKCH:    clk->ops = &ops_clkch;   break;
		case CLK_TYPE_CLKEN:    clk->ops = &ops_clken;   break;
		case CLK_TYPE_CLKDIV:   clk->ops = &ops_clkdiv;  break;
		default:
			return NULL;
	}
	...
	if (plen > 0) {
		int phandle = fdt32_to_cpu(ac[0]);
		int idx = 0;
		if (plen > sizeof(uint32_t)) {
			idx = fdt32_to_cpu(ac[1]);
		}
	...
	/* freq */
	clk->freq = clk->ops->get(base);

	/* info */
	VERBOSE("%s: base=0x%lx, name=%s, is_en=%d, freq=%lld\n", __func__,
		clk->base,
		clk->name,
...
int cmu_desc_init(void)
{
	int offs = 0;
	int i;
	void *fdt = (void *)(uintptr_t)PLAT_BAIKAL_NS_DTB_BASE;
	int ret = fdt_open_into(fdt, fdt, PLAT_BAIKAL_DTB_MAX_SIZE);
	if (ret < 0) {
...
int cmu_calc_range(int64_t freq)
{
	...
	if (freq > 200 * MHZ){
		return -1;
	     ...
	     if (freq > 130 * MHZ)	range = 111;
	else if (freq > 80  * MHZ)	range = 110;
	else if (freq > 50  * MHZ)	range = 101;
	else if (freq > 30  * MHZ)	range = 100;
	else if (freq > 18  * MHZ)	range = 011;
	else if (freq > 11  * MHZ)	range = 010;
	else if (freq > 7   * MHZ)	range = 001;
	else				range = 000;    // bypass
	return range;
}
...
int cmu_desc_set_freq(uintptr_t base, int64_t freq)
{
	struct clk_desc *clk = cmu_desc_get_by_base(base);
	if (!clk){
		return -1;
...
int64_t cmu_desc_get_freq(uintptr_t base)
{
	struct clk_desc *clk = cmu_desc_get_by_base(base);
	if (!clk){
		return -1;
...
int cmu_pll_div_set(uintptr_t base, void *div)
{
	uint32_t raw;
...
int cmu_pll_div_get(uintptr_t base, void *div)
{
	int ret;
...
int cmu_pll_div_calc(int64_t freq, int64_t fref, void *div)
{
	// fout = 2*(fref*NF)/(NR*OD)
	int ret = -1;
	int64_t fout = -1;
...
int cmu_clkch_div_set(uintptr_t base, void *div)
{
	uint32_t raw;
...
int cmu_clkch_div_get(uintptr_t base, void *div)
{
	int ret;
	ret = reg_read(base,div);
	if (ret < 0){
...
int cmu_clkch_div_calc(int64_t freq, int64_t fref, void *div)
{
	// fout = fref/(VAL_CLKDIV+1)
	int ret = -1;
	int64_t fout = -1;
...
int cmu_clkdiv_div_set(uintptr_t base, void *div)
{
	uint32_t raw;
...
int cmu_clkdiv_div_get(uintptr_t base, void *div)
{
	int ret;
	ret = reg_read(base,div);
	if (ret < 0){
...
int cmu_clkdiv_div_calc(int64_t freq, int64_t fref, void *div)
{
	// fout = fref/2^(DIV_VAL-1)
	int ret = -1;
	int64_t fout = -1;
...
int cmu_clken_div_set(uintptr_t base, void *div)
{
	uint32_t raw;
	cmu_clkdiv_t *reg = (void*)&raw;
	cmu_clkdiv_t *new = div;
...
int cmu_clken_div_get(uintptr_t base, void *div)
{
	int ret;
	ret = reg_read(base,div);
	if (ret < 0){
...
int cmu_clken_div_calc(int64_t freq, int64_t fref, void *div)
{
	// fout = fref/DIV_VAL
	int ret = -1;
	int64_t fout = -1;
...
int64_t cmu_clkref_get_rate(uintptr_t base)
{
	...
	*/
	// регистр УПРАВЛЕНИЯ глобальной загрузкой
	...
	*/
	...
	cmu_desc_set_freq(base,freq);
	return freq;
}
int cmu_clkref_set_rate(uintptr_t base, int64_t freq)
{
	return -1;
}
int cmu_clkref_disable(uintptr_t base)
{
	return -1;
}
int cmu_clkref_enable(uintptr_t base)
{
	return -1;
}
int cmu_clkref_is_enabled(uintptr_t base)
{
	return 1;
}
...
int cmu_pll_set_rate(uintptr_t base, int64_t freq)
{
	uint32_t raw;
	cmu_pll_t *reg = (void*)&raw;
	int ret;
...
int cmu_clkch_set_rate(uintptr_t base, int64_t freq)
{
	uint32_t raw;
	cmu_clkch_t *reg = (void*)&raw;
	int ret;
...
int cmu_clkch_disable(uintptr_t base)
{
	// CLKEN = 0
	uint32_t raw;
	cmu_clkch_t *reg = (void*)&raw;
...
int cmu_clkch_enable(uintptr_t base)
{
	// CLKEN = 1
	uint32_t raw;
	cmu_clkch_t *reg = (void*)&raw;
...
int cmu_clkch_is_enabled(uintptr_t base)
{
	// return CLKEN
	uint32_t raw;
	cmu_clkch_t *reg = (void*)&raw;
...
int cmu_clkdiv_set_rate(uintptr_t base, int64_t freq)
{
	uint32_t raw;
	cmu_clkdiv_t *reg = (void*)&raw;
	int ret;
	...
	if (cmu_clkdiv_div_set(base,&div)){
		return -1;
	}
	...
	// 5. CLKEN = 1
	cmu_clkdiv_enable(base);

	freq = cmu_clkdiv_calc(fref,&div);
	cmu_desc_set_freq(base,freq);
	return 0;
}
int cmu_clkdiv_disable(uintptr_t base)
{
	// CLKEN = 0
	uint32_t raw;
	cmu_clkdiv_t *reg = (void*)&raw;
...
int cmu_clkdiv_enable(uintptr_t base)
{
	// CLKEN = 1
	uint32_t raw;
	cmu_clkdiv_t *reg = (void*)&raw;
...
int cmu_clkdiv_is_enabled(uintptr_t base)
{
	// return CLKEN
	uint32_t raw;
	cmu_clkdiv_t *reg = (void*)&raw;
...
int cmu_clken_set_rate(uintptr_t base, int64_t freq)
{
	uint32_t raw;
...
int cmu_clken_disable(uintptr_t base)
{
	// CLKEN = 0
	uint32_t raw;
	cmu_clkdiv_t *reg = (void*)&raw;
...
int cmu_clken_enable(uintptr_t base)
{
	// CLKEN = 1
	uint32_t raw;
	cmu_clkdiv_t *reg = (void*)&raw;
...
int cmu_clken_is_enabled(uintptr_t base)
{
	// return CLKEN
	uint32_t raw;
	int ret;
...
// SMC
//--------------------
int baikal_smc_clk_handler  (const uint32_t ops,
			     const uint64_t base,
			     const uint64_t freq,
			     const uint64_t x3,
			     const uint64_t x4)
{
	...
	switch (ops) {
		case BAIKAL_SMC_CLK_GET:
			ret = clk->ops->get(base);
			break;
		case BAIKAL_SMC_CLK_SET:
			ret = clk->ops->set(base,freq);
			break;
		case BAIKAL_SMC_CLK_ROUND:
			ret = clk->ops->round(base,freq);
			break;
		case BAIKAL_SMC_CLK_DISABLE:
			ret = clk->ops->disable(base);
			break;
		case BAIKAL_SMC_CLK_ENABLE:
			ret = clk->ops->enable(base);
			break;
		case BAIKAL_SMC_CLK_IS_ENABLED:
			ret = clk->ops->is_enabled(base);
			break;
	}
	return ret;
}
