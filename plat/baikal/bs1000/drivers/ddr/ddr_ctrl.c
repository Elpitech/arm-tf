...
void umctl2_enter_SR(int port)
{
	...
	reg  = BS_DDRC_READ(port, PWRCTL);
	reg |= (1 << 5);
	BS_DDRC_WRITE(port, PWRCTL, reg);
...
{
	uint32_t chip_offset = PLATFORM_CHIP_MEM_OFFSET(DDR_PORT_CHIP(port)) >> 28;

	/* program SAR registers (UMCTL2_SARMINSIZE = 256 MiB) */
	/* configure a SARBASE0 reg. */
	BS_DDRC_WRITE(port, SARBASE0, chip_offset + 2 * 4); /* region 0: 2 GiB base address */
	/* configure a SARSIZE0 reg. */
	BS_DDRC_WRITE(port, SARSIZE0, 2 * 4 - 1); /* region 0: 2 GiB size */

	/* configure a SARBASE1 reg. */
	BS_DDRC_WRITE(port, SARBASE1, chip_offset + 34 * 4); /* region 1: 34 GiB base address */
	/* configure a SARSIZE1 reg. */
	BS_DDRC_WRITE(port, SARSIZE1, 30 * 4 - 1); /* region 1: 30 GiB size */

	/* configure a SARBASE2 reg. */
	BS_DDRC_WRITE(port, SARBASE2, chip_offset + 544 * 4); /* region 2: 544 GiB base address */
	...
	BS_DDRC_WRITE(port, SARBASE3, chip_offset + 608 * 4); /* region 3: 544 + 64 GiB base address */
...
int ctrl_init(int port, struct ddr_configuration *data)
{
	...
	reg |= 1 << 4; /* ctl operates as ddr4 */
	reg |= GENMASK(19, 16) & (0x4 << 16); /* BL8 */
	/*
	...
	if (data->device_width == 8) {
		reg |= GENMASK(31, 30) & (1 << 30);
	}
	BS_DDRC_WRITE(port, MSTR, reg);
	...
	reg = (1 << 0); /* Send MRS commands to each ranks separately */
	...
	BS_DDRC_WRITE(port, DIMMCTL, reg);
	...
	reg  = (data->WL - 5 + data->registered_dimm); /* see PHY PUB DataBook, Table5-3 (p.91) */
	reg |= (2 << 8); /* see PHY PUB DataBook, Table5-3 (p.91) */
	reg |= (1 << 15);
	reg |= ((data->RL - 5 + data->registered_dimm) << 16); /* see PHY PUB DataBook, Table5-5 (p.92) */
	reg |= (1 << 23);
	reg |= (((6 + data->registered_dimm + 1) / 2) << 24); /* see PHY PUB DataBook, Table5-1 (p.90) */
	BS_DDRC_WRITE(port, DFITMG0, reg);

	/* configure a DFITMG1 reg. */
	reg  = 2; /* see PHY PUB DataBook, Table5-12 (p.97) */
	reg |= (2 << 8); /* see PHY PUB DataBook, Table5-12 (p.97) */
	 /* see PHY PUB DataBook, Table5-3 (p.91) << 16); (t_ctrl_delay + (6 + 4) + TxDqsDly) / 2 */
	reg |= GENMASK(20, 16) & ((((DDR_PHY_TWRDATA_DELAY + 1) / 2 + 1)) << 16);
	/* add one additional clk for RDIMM (see F0RC0F settings: 1 clk delay for parity by default) */
	reg |= ((data->registered_dimm ? 1 : 0) << 24);
	reg &= ~GENMASK(31, 28); /* note: have to set non-zero value for CAL mode (we don't use CAL mode) */
	const uint32_t dfitmg1_val = reg;

	BS_DDRC_WRITE(port, DFITMG1, reg);

	/* configure a DFITMG2 reg. */
	reg  = GENMASK(5, 0) & (data->WL - 5 + data->registered_dimm); /* see PHY PUB DataBook, Table5-3 (p.91) */
	reg |= GENMASK(14, 8) & ((data->RL - 5 + data->registered_dimm) << 8); /* see PHY PUB DataBook, Table5-5 (p.92) */
	BS_DDRC_WRITE(port, DFITMG2, reg);

	/* configure a DRAMTMG0 reg. */
	unsigned int t_ras_min = data->timing_2t ? (data->tRAS + 1) / 2 : data->tRAS / 2;

	reg  = GENMASK(5, 0) & t_ras_min;
	reg |= GENMASK(14, 8) & (((data->tRASmax / 1024 - 1) / 2) << 8);
	reg |= GENMASK(21, 16) & (((data->tFAW + 1) / 2) << 16);
	unsigned int wr2pre = (data->WL + 4 + data->tWR);

	reg |= GENMASK(30, 24) & ((data->timing_2t ? (wr2pre + 1) / 2 : wr2pre / 2) << 24);
	BS_DDRC_WRITE(port, DRAMTMG0, reg);

	/* configure a DRAMTMG1 reg. */
	reg = GENMASK(6, 0) & ((data->tRC + 1) / 2);
	unsigned int rd2pre = ((data->AL + data->tRTP) > (data->RL + 4 - data->tRP)) ?
							(data->AL + data->tRTP) : (data->RL + 4 - data->tRP);
	reg |= GENMASK(13, 8) & ((data->timing_2t ? (rd2pre + 1) / 2 : rd2pre / 2) << 8);
	reg |= GENMASK(20, 16) & (((data->tXP + data->PL + 1) / 2) << 16);
	BS_DDRC_WRITE(port, DRAMTMG1, reg);

	/* configure a DRAMTMG2 reg. */
	unsigned int wr2rd = data->CWL + data->PL + 4 + data->tWTR_L; /* CRC off */

	reg  = GENMASK(5, 0) & ((wr2rd + 1) / 2);
	/* see umctl2 Databook for "rd2wr" description */
	reg |= GENMASK(13, 8) & (((data->RL + 4 + 1 + (data->wr_preamble_2CK ? 2 : 1) - data->WL + 1) / 2) << 8);
	reg |= GENMASK(21, 16) & (((data->RL + 1 + data->registered_dimm) / 2) << 16);
	reg |= GENMASK(29, 24) & (((data->WL + 1 + data->registered_dimm) / 2) << 24);
	BS_DDRC_WRITE(port, DRAMTMG2, reg);

	/* configure a DRAMTMG3 reg. */
	reg  = GENMASK(9, 0) & (DIV_ROUND_UP_2EVAL(data->tMOD + data->PL, 2) + !!data->registered_dimm);
	reg |= GENMASK(17, 12) & (DIV_ROUND_UP_2EVAL(data->tMOD + data->PL, 2) << 12); /* tMRD_PAR */
	BS_DDRC_WRITE(port, DRAMTMG3, reg);

	/* configure a DRAMTMG4 reg. */
	reg  = GENMASK(4, 0) & (data->tRP / 2 + 1);
	reg |= GENMASK(11, 8) & (((data->tRRD_L + 1) / 2) << 8);
	reg |= GENMASK(19, 16) & (((data->tCCD_L + 1) / 2) << 16);
	reg |= GENMASK(28, 24) & (((data->tRCD > data->AL) ? (data->tRCD - data->AL + 1) / 2 : 1) << 24);
	BS_DDRC_WRITE(port, DRAMTMG4, reg);
	...
	BS_DDRC_WRITE(port, DRAMTMG5, reg);

	/* configure a DRAMTMG8 reg. */
	reg  = GENMASK(6, 0) & (((data->tXS + 31) / 32 + 1) / 2 + 1); /* note: we see bug by VCS testing: */
	/* tXS = 320 tCK, but measured delay is 306 tCK */
	reg |= GENMASK(14, 8) & ((((data->tXSDLL + 31) / 32 + 1) / 2 + 1) << 8);
	reg |= GENMASK(22, 16) & ((((data->tXS_ABORT + 31) / 32 + 1) / 2 + 1) << 16); /* add +1 for all register fields */
	reg |= GENMASK(30, 24) & ((((data->tXS_FAST + 31) / 32 + 1) / 2 + 1) << 24);
	BS_DDRC_WRITE(port, DRAMTMG8, reg);

	/* configure a DRAMTMG9 reg. */
	/* note: don't correct wr2rd_s timing by PHY training results: umctl2 databook don't say nothing */
	reg  = GENMASK(5, 0) & ((data->CWL + data->PL + 4 + data->tWTR_S + 1) / 2);
	reg |= GENMASK(11, 8) & (((data->tRRD_S + 1) / 2) << 8);
	reg |= GENMASK(18, 16) & (((data->tCCD_S + 1) / 2) << 16);
	reg |= GENMASK(30, 30) & ((data->wr_preamble_2CK ? 1 : 0) << 30); /* Write Preamble = 1tCK / 2 tCK */
	BS_DDRC_WRITE(port, DRAMTMG9, reg);

	/* configure a DRAMTMG11 reg. */
	reg  = GENMASK(4, 0) & ((data->tCKMPE + 1) / 2);
	reg |= GENMASK(9, 8) & (((data->tMPX_S + 1) / 2) << 8);
	reg |= GENMASK(20, 16) & ((data->tMPX_LH / 2 + 1) << 16);
	reg |= GENMASK(30, 24) & ((((data->tXMPDLL + 31) / 32 + 1) / 2) << 24);
	BS_DDRC_WRITE(port, DRAMTMG11, reg);

	/* configure a DRAMTMG12 reg. */
	reg  = GENMASK(4, 0) & ((data->tMRD_PDA + 1) / 2);
	reg |= GENMASK(29, 24) & (((data->tMOD + data->PL + data->AL + 1) / 2) << 24);
	BS_DDRC_WRITE(port, DRAMTMG12, reg);
	...
	reg  = GENMASK(9, 0) & ((data->tRFC1 + 1) / 2); /* tRFC1 for Refresh Mode normal(1x)); */
	reg |= GENMASK(27, 16) & ((data->tREFI / 32 / 2) << 16);
	BS_DDRC_WRITE(port, RFSHTMG, reg); /* 7800; 350 */
	...
	reg  = GENMASK(6, 2) & (rd_odt_delay << 2);
	reg |= GENMASK(11, 8) & ((5 + (data->rd_preamble_2CK ? 2 : 1)) << 8); /* DDR4 BL8: 5 + RD_PREAMBLE */
	reg |= GENMASK(20, 16) & (dfi_t_cmd_lat << 16);
	/* DDR4 BL8: 5 + WR_PREAMBLE + CRC_MODE */
	reg |= GENMASK(27, 24) & ((5 + (data->wr_preamble_2CK ? 2 : 1) + (data->crc_on ? 1 : 0)) << 24);
	BS_DDRC_WRITE(port, ODTCFG, reg); /* cons. using wr_preamble */

	/* configure a ODTMAP reg. */
	if (data->dimms == 1) {
		/* ODT signal mapping: single DIMM */
		if (data->ranks == 1) {
			reg = 0x00000201; /* single DIMM, 1-rank ODT mask */
		} else {
			reg = 0x00000102; /* single DIMM, 2-rank ODT mask */
		}
	} else {
		/* ODT signal mapping: dual DIMM */
		if (data->ranks == 1) {
			reg = 0x00010004; /* dual DIMMs, 1-rank ODT mask */
		} else {
			reg = 0x22228888; /* dual DIMMs, 2-rank ODT mask */
		}
	}
	BS_DDRC_WRITE(port, ODTMAP, reg);

	/* Program DQ Map */
	/* configure a DQMAP0 reg. */
	BS_DDRC_WRITE(port, DQMAP0, data->DQ_map[0]);
	/* configure a DQMAP1 reg. */
	BS_DDRC_WRITE(port, DQMAP1, data->DQ_map[1]);
	...
	reg  = BS_DDRC_READ(port, SCHED);
	reg |= (1 << 2);
	reg &= ~GENMASK(12, 8);
	reg |= GENMASK(12, 8) & (DDR_CTRL_QOS_LPR << 8); /* uMCTL2 QoS: LPR storage size */
	BS_DDRC_WRITE(port, SCHED, reg);
