...
{
	uint8_t *dst = (void *)PLAT_BAIKAL_SEC_DTB_BASE;
	int err;
	const struct fdt_header *const fdt_header = (void *)dst;
	uint32_t fdt_totalsize;

	err = dw_spi_read(0, flashaddr, dst, sizeof(struct fdt_header));
	...
	if (fdt_totalsize < sizeof(struct fdt_header) ||
	    fdt_totalsize > PLAT_BAIKAL_DTB_MAX_SIZE) {
	...
	err = dw_spi_read(0, flashaddr, dst, fdt_totalsize - sizeof(struct fdt_header));
	...
	INFO("DTB: crc32:0x%08x size:%u\n",
		crc32((void *)PLAT_BAIKAL_SEC_DTB_BASE, fdt_totalsize, 0),
		fdt_totalsize);
...
static int read_fip_from_flash(uintptr_t local_image_handle, uint32_t offset)
{
	...
	INFO("FIP: crc32:0x%08x size:%lu\n",
		crc32((void *)PLAT_SEC_FIP_BASE, (uintptr_t)addr + size - PLAT_SEC_FIP_BASE, 0),
		(uintptr_t)addr + size - PLAT_SEC_FIP_BASE);
...
static int read_flash(uintptr_t local_image_handle)
{
	...
	result  = read_fdt_from_flash(BAIKAL_FLASH_MAP_FDT);
