...
{
	uint8_t *addr = (uint8_t *)PLAT_BAIKAL_DT_BASE;
	int src = offset;
	int result;
...
static int read_flash(uintptr_t local_image_handle)
{
	...
	result |= read_fdt_from_flash(local_image_handle, FLASH_MAP_FDT);
	result |= read_fip_from_flash(local_image_handle, FLASH_MAP_FIP);
