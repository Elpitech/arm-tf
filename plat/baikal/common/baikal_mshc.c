...
static void reg_write(const uintptr_t base, const int val, const int reg)
{
	switch (reg_size(reg)) {
	case 4:
		...
		break;
	case 2:
		mmio_write_16(base + reg, val);
		break;
	case 1:
		mmio_write_8(base + reg, val);
		break;
	}
}

static int reg_read(const uintptr_t base, const int reg)
{
	switch (reg_size(reg)) {
	case 4:
		return mmio_read_32(base + reg);
	case 2:
		return mmio_read_16(base + reg);
	case 1:
		return mmio_read_8(base + reg);
	}
...
static int dw_send_cmd(struct mmc_cmd *cmd)
{
	...
	/* TODO: why are the resp_data should be shifted by 1 byte for CMD9 ? */
	if (Cmd->index == MMC_CMD(9)) {
		memmove((uint8_t *)cmd->resp_data + 1, cmd->resp_data, sizeof(cmd->resp_data) - 1);
		*(uint8_t *)cmd->resp_data = 0;
	}
...
static int dw_read(int lba, uintptr_t buf, size_t size)
{
	...
	for (j = 0; j < Blocks; j++) {
		...
		for (Iter = 0; Iter < Blocksize / sizeof(uint32_t); Iter++) {
			*P++ = mmio_read_32(base + SDHCI_BUFFER);
		}
	}
...
static int dw_write(int lba, uintptr_t buf, size_t size)
{
	...
	for (j = 0; j < Blocks; j++) {
		...
		for (Iter = 0; Iter < Blocksize / sizeof(uint32_t); Iter++) {
			mmio_write_32(base + SDHCI_BUFFER, *P++);
		}
	}
