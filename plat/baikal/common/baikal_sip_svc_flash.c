...
static int baikal_smc_flash_init(uint64_t *data);
static int baikal_smc_flash_position(unsigned int pos);
static int baikal_smc_flash_pull(uint64_t *data);
static int baikal_smc_flash_push(uint64_t data0,
				 uint64_t data1,
				 uint64_t data2,
				 uint64_t data3);
...
{
	bootflash_init();
	data[0] = BAIKAL_BOOTFLASH_SUBSECTOR;
	data[1] = BAIKAL_BOOTFLASH_SIZE / BAIKAL_BOOTFLASH_SUBSECTOR;
	return 0;
}
