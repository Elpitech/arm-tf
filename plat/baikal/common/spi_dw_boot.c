...
#define SPI_TXFTLR	*(volatile uint32_t *)(BAIKAL_SPI_BASE + 0x18)
#define SPI_RXFTLR	*(volatile uint32_t *)(BAIKAL_SPI_BASE + 0x1c)
...
#define SPI_DFS_SET(x)						\
do {								\
	uint32_t reg = SPI_CTRLR0;				\
	reg &= ~SPI_DFS_MASK;					\
	reg |= (SPI_DFS(x) << SPI_DFS_OFFSET) & SPI_DFS_MASK;	\
	SPI_CTRLR0 = reg;					\
} while (0)
		    ...
		    void *rx_, uint32_t rx_len)
{
	...
	gpio32_out_rst(BAIKAL_BOOT_SPI_CS_GPIO_PIN);
	gpio32_dir_set(BAIKAL_BOOT_SPI_CS_GPIO_PIN);
#endif
	...
	if (rx_len) {
		SPI_CTRLR0 |= SPI_TMOD_EEPROMREAD << SPI_TMOD_OFFSET;
	} else {
	...
	case SPI_TMOD_TX:
		...
		while (cmd < cmdend && (SPI_SR & SPI_SR_TFNF)) {
			SPI_DR = *cmd++;
		}
		while (tx < txend && (SPI_SR & SPI_SR_TFNF)) {
			SPI_DR = *tx++;
		}

		SPI_SER = line; /* start sending */
...
exit:
#ifdef BAIKAL_BOOT_SPI_CS_GPIO_PIN
	gpio32_out_set(BAIKAL_BOOT_SPI_CS_GPIO_PIN);
#endif
	if (err) {
	...
	int baudr)
{
	...
	return transfer(line, cmd, lencmd, in, lenin, out, lenout);
}

static int wait_tx(void)
{
	volatile int status;
	uint64_t timeout = timeout_init_us(SPI_TIMEOUT_BAUDR(SPI_BAUDR, SPI_DFS_GET, 50));

	for (;;) {
		status = SPI_SR;
		if (status & SPI_SR_TFNF) {
			return 0;
		}
		if (status & SPI_SR_DCOL) {
			return -EBADMSG;
		}
...
const struct flash_info *dw_spi_get_info(int line)
{
	...
	for (i = 0; i < ARRAY_SIZE(spi_flash_ids); ++i) {
		...
		if (!memcmp(info->id, id, sizeof(id))) {
			INFO("SPI Flash: JEDEC ID 0x%02x%02x%02x, total %u MiB\n",
			     id[0], id[1], id[2],
